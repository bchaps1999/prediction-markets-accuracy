...
ui <- navbarPage(
    ...
    tabPanel("State Data",
             sidebarLayout(
                 sidebarPanel(
                     ...
                     p(
                         "This project uses data from 59 primary elections and caucuses in 2016 and 2020. Select a state and election year to see how candidates' polling averages and market prices changed over time. Candidates are only displayed for a given state if they did not drop out before the date of the election."
                     )
                 ),
    ...
    tabPanel(
        ...
        sidebarLayout(sidebarPanel(
            ...
            p(
                "Select a prediction type to view the final data for that variable. Every point on the plot represents a candidate in a specific state and election year. The predictor variables, either final market price or final polling average, are plotted against the vote percent in the election for each candidate. These are final values in the sense that they are taken from the day before each election."
            ),
            p(
                "Regression lines are also generated for each plot to better understand the relationship between the two variables. A linear regression model was used for the polling average data, while a third-degree polynomial regression was used for the prediction market data. The model for the polling average data displays a higher R squared value than the model for the prediction market data, indicating that the polling average data fits its model more closely."
            )
        ),
    ...
    tabPanel(
        ...
        column(6,
               wellPanel(
                   ...
                   br(),
                   p(
                       "To determine how accurate the two variables were at predicting election results, new variables were created to represent whether or not the prediction markets or polls predicted that a specific candidate would win. These variables were then compared to the actual election outcomes to determine whether or not each prediction was correct and calculate an overall percent accuracy. As can be seen in the table, the prediction markets were seven percentage points more accurate than the polling averages. In total, the markets correctly predicted 4 more elections than the polls. "
                   ),
                   p(
                       "It is important to note, however, that these predictions are based on the final market prices and polling averages from the day before the election. These levels of accuracy change over the weeks leading up to the election, as can be seen on the Predictions Over Time page."
                   )
               )),
        column(6,
               wellPanel(
                 ...
                 wellPanel(
                     ...
                     p(HTML(
                         paste0(
                             ...
                             a(href = 'https://github.com/fivethirtyeight/data/tree/master/polls', 'FiveThirtyEight\'s estimated polling average'),
                             ' for each candidate. The prediction market data is from ',
                             a(href = 'predictit.org', 'PredictIt.org'),
                             ', and the daily closing market price will be used as the default value. Election results are from the ',
                             a(href = 'https://www.nytimes.com/news-event/2020-election', 'New York Times'),
                             ' and various state websites. Candidates were included in the data set only if they had sufficient market, polling, and election results data for a specific state. States were only included if there was more than one remaining candidate with enough data. As a result, only 59 primaries and caucuses from the two election years are included in the data.'
                         )
                     )),
                     h3("About the Author"),
                     p(
                         "Brendan Chapuis is a sophomore at Harvard College studying government and data science with a minor in economics."
                     ),
                     h4("Contact Information"),
                     tags$a(href = "https://github.com/bchaps1999", "GitHub"),
                     br(),
                     tags$a(href = "https://www.linkedin.com/in/brendan-chapuis-341703160/", "LinkedIn")
                 )
             ))
)
